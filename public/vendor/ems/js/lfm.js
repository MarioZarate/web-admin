(function( $ ){
  ...
  $.fn.filemanager = function(type, options) {
    ...
    this.on('click', function(e) {
      ...
      window.open(route_prefix + '?type=' + type, 'FileManager', 'titlebar=no,toolbar=no,location=no,status=no,menubar=no,width=900,height=600');
      return false;
    });
    ...
    $(document).on('click', '.fm-button' , function(e){
        localStorage.setItem('target_input', $(this).data('input'));
        localStorage.setItem('target_preview', $(this).data('preview'));
        window.open('/laravel-filemanager?type=' + type, 'FileManager', 'titlebar=no,toolbar=no,location=no,status=no,menubar=no,width=900,height=600');
        return false;
    })
  }
...
function SetUrl(url, file_path){
  ...
  if (extension.toLowerCase() == 'png' ||  extension.toLowerCase() == 'jpg') {
      ...
      target_input.closest(".ui.fluid").find('.delete-flm').remove();
      ...
      if (exists_preview == 1) {
          target_input.closest(".ui.fluid").find('.preview-flm').remove();
          target_input.closest(".imagen-array").find('.title-image-flm').text(image_value)
          target_input.closest(".ui.fluid").append('<a class="btn btn-danger btn-lg delete-flm" data-name="'+file_path+'"><i class="fa fa-trash"> </i></a>')
        }else{
          target_input.closest(".imagen-array").find('.title-image-flm').text(image_value)
